...
class TelnetTestWidget(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.telnet_btn = Button(text='开始 Telnet', font_size='16sp', bold=True)
        self.telnet_btn.bind(on_press=self.start_telnet)
        self.stop_btn = Button(text='停止', font_size='16sp', state='disabled')
        self.stop_btn.bind(on_press=self.stop_telnet)
        btn_layout.add_widget(self.telnet_btn)
    ...
    def start_telnet(self, instance):
        ...
        self._running = True
        self.telnet_btn.state = 'disabled'
        self.stop_btn.state = 'normal'
        self.progress.show()
        self.progress.value = 0
        self.result_label.text = f'正在连接 {host}:{port} ...'
        self.result_label.color = (0.2, 0.6, 0.2, 1)

        thread = threading.Thread(target=self._run_telnet, args=(host, port, timeout))
        thread.daemon = True
        thread.start()

    def stop_telnet(self, instance):
        self._running = False
        if self._socket:
            try:
                self._socket.close()
    ...
    def _run_telnet(self, host, port, timeout):
        try:
            start_time = time.time()
            output_lines = [f'尝试连接 {host}:{port} ...']
            ...
            try:
                ...
                Clock.schedule_once(lambda dt: self.finish_telnet('\n'.join(output_lines), False), 0)
                return
            ...
            try:
                ...
                output_lines.append(f'✅ 连接成功! ({conn_time:.2f} ms)')
                ...
                try:
                    ...
                    if banner:
                        output_lines.append(f'Banner: {banner}')
                ...
                output_lines.append('端口可能关闭或被防火墙阻止')
                Clock.schedule_once(lambda dt: self.finish_telnet('\n'.join(output_lines), False), 0)
            except ConnectionRefusedError:
                conn_time = (time.time() - conn_start) * 1000
                output_lines.append(f'❌ 连接被拒绝 ({conn_time:.2f} ms)')
                output_lines.append('目标端口未开放或服务未运行')
                Clock.schedule_once(lambda dt: self.finish_telnet('\n'.join(output_lines), False), 0)
            except OSError as e:
            ...
            Clock.schedule_once(lambda dt: self.finish_telnet(error_msg, False), 0)
        finally:
            if self._socket:
                try:
                    self._socket.close()
    ...
    def finish_telnet(self, result, success):
        self.result_label.text = result
        if success:
            self.result_label.color = (0.2, 0.6, 0.2, 1)
        else:
            self.result_label.color = (1, 0.3, 0.3, 1)
        self.telnet_btn.state = 'normal'
